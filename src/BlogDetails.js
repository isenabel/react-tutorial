...
const BlogDetails = () => {
  const { id } = useParams();
  const { data: blog, isPending, error } = useFetch('http://blog-backend-dev.us-east-2.elasticbeanstalk.com/blogs/' + id);
  ...
  const deleteBlog = async () => {
    await fetch('http://blog-backend-dev.us-east-2.elasticbeanstalk.com/blogs/' + id, {
      method: 'DELETE'
    })
      .then(async (res) => {
